class Globe {
    ...
    initializeGlobe() {
        // call's the WebGL-Earth library's initialize() function
        const globe = new WE.map("globe-div", { sky: true });
        ...
        // Mostly copied from the documentation found here => http://examples.webglearth.com/#animation
        var before = null;
        requestAnimationFrame(function animate(now) {
            // These 2 lines aren't in the documentation
            // I added it to change rotation speed depending on zoom level in order to make it easier to click markers when zoomed in
            ...
            const rotationSpeed = Math.min( 0.1, 1 / ( 2 * z * z ) );
    ...
    // create green active and grey inactive markers - images from https://github.com/pointhi/leaflet-color-markers
    ...
    // create markers for each location in each article and place them in an object to be retrieved later
        ...
        const displayMarkers = (articles, iconImage) => {
            for (const article of articles) {
                for (const marker of this.allMarkers[ article.url ]) {
                    ...
                    marker.element.querySelector(".we-pm-icon").style.backgroundImage = `url("${iconImage}")`;
